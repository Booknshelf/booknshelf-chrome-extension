...
let iframeGrimReaper = function () {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
        chrome.tabs.sendMessage(tabs[0].id, { trigger: constants_1.Trigger.click_outside });
    });
};
...
ui_1.setLoadingScreenText("Searching for book...");
m.mount(document.body, ui_1.loadingScreen);
setTimeout(function () {
    ui_2.setQueryString(queryString);
    ui_2.setBooks(sampleBooks);
    m.mount(document.body, ui_2.selectBookScreen);
    ui_2.setOnClickBook(function (book) {
        console.log("Book clicked : " + book[1]);
        ui_3.setSelectedBook(book);
        ui_3.setBookshelfs(sampleBookshelfs);
        m.mount(document.body, ui_3.selectListScreen);
        ui_3.setOnClickSave(function (bookshelf) {
            console.log("Book " + book[1] + " saved to " + bookshelf);
            ui_1.setLoadingScreenText("Saving book...");
            m.mount(document.body, ui_1.loadingScreen);
